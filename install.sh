...
if [ "$UID" -eq "$ROOT_UID" ]; then
  ...
  grep "GRUB_TIMEOUT=" /etc/default/grub 2>&1 >/dev/null && sed -i '/GRUB_TIMEOUT=/d' /etc/default/grub
  echo "GRUB_TIMEOUT=${THEME_TIMEOUT}" >> /etc/default/grub

  grep "GRUB_SAVEDEFAULT=" /etc/default/grub 2>&1 >/dev/null && sed -i '/GRUB_SAVEDEFAULT=/d' /etc/default/grub
  echo "GRUB_SAVEDEFAULT=true" >> /etc/default/grub

  grep "GRUB_TERMINAL_OUTPUT=" /etc/default/grub 2>&1 >/dev/null && sed -i '/GRUB_TERMINAL_OUTPUT=/d' /etc/default/grub
  ...
  echo "GRUB_GFXMODE=${THEME_RESOLUTION}" >> /etc/default/grub
